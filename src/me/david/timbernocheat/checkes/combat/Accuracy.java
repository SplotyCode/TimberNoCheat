...
public class Accuracy extends Check {
    ...
    private boolean check(final Player player, final Player target, boolean miss) {
        if(target != null){
            if(!CheckManager.getInstance().isvalid_create(target))return false;
            long moveDelay = System.currentTimeMillis()-CheckManager.getInstance().getPlayerdata(target).getGenerals().getLastRealMove();
